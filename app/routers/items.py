...
@router.get(
    ...
    dependencies=[Depends(require_permission(Permissions.ITEMS_READ))],
)
def read_item(
...
@router.put(
    "/{tenant_id}/items/{item_id}",
    dependencies=[Depends(require_permission(Permissions.ITEMS_UPDATE))],
)
def update_item(
    tenant: Tenant = Depends(get_current_tenant),
    db: Session = Depends(get_db),
...
@router.delete(
    "/{tenant_id}/items/{item_id}",
    dependencies=[Depends(require_permission(Permissions.ITEMS_DELETE))],
)
def delete_item(
    tenant: Tenant = Depends(get_current_tenant),
    db: Session = Depends(get_db),
